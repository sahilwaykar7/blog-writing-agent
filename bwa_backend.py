...
def _openai_generate_image_bytes(prompt: str, size: str = "1024x1024", quality: str = "medium") -> bytes:
    """
    ...
    if not b64_json:
        raise RuntimeError("No base64 image payload returned from OpenAI.")
...
def _generate_image_bytes_with_fallback(spec: dict) -> bytes:
    prompt = spec["prompt"]
    ...
    try:
        return _openai_generate_image_bytes(prompt=prompt, size=size, quality=quality)
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"{openai_error}. Gemini fallback failed: {e}"
        ) from e
...
def generate_and_place_images(state: State) -> dict:
    ...
    for spec in image_specs:
        ...
        if not out_path.exists():
            try:
                img_bytes = _generate_image_bytes_with_fallback(spec)
